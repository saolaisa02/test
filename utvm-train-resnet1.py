...
def res_block_mish(x,sz,filter_sz=3):

    xi  = Mish()(x)
    x_linear = x #get lineaer
...
def create_model_resnet1():
    print("Model cus resnet1 mish\n--------------------------------------------------------")
    ...
    x = Conv2D(32, 3, activation='linear', padding='same', strides=(1,1))(x)
    x = res_block_mish(x,32)
    x = res_block_mish(x,32)
    
    x = Conv2D(64, 3, activation='linear', padding='same', strides=(1,1))(x)
    x = res_block_down(x,64)
    x = res_block_mish(x,64)
                               
    x = Conv2D(128, 3, activation='linear', padding='same', strides=(1,1))(x)
    x = res_block_down(x,128)
    x = res_block_mish(x,128) 
    
    x = Conv2D(256, 3, activation='linear', padding='same', strides=(1,1))(x)
...
#             train_data = read_data("/data/utvm/anhnt/utvm_data_split/bienso_updated_1002.txt")
#             test_data = read_data("/data/utvm/anhnt/utvm_data_split/bienso_updated_1002.txt")
...
#             train_data = read_data("/data/anhnt/utvm_data_split/stanford/bienso_stanford_train.txt")
#             test_data = read_data("/data/anhnt/utvm_data_split/stanford/bienso_stanford_test.txt")
...
if __name__ == '__main__':
    main()
''' 
python utvm-train-resnet1.py --model models_official/utvm_models_resnet1_private_data/utvm-model-privatedata-resnet1_backup_max --name utvm-model-privatedata-resnet1 --output-dir models_official/utvm_models_resnet1_private_data -op Adam -lr .0001 -its 100000 -bs 32
models_official/utvm_models_resnet1_private_data/utvm-model-private-data-resnet1_backup_max
