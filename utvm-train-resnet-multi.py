...
def res_block_mish_multi(x,sz,filter_sz=3, down=False, up_size=False, pooling=False):
    if(up_size==True):
    ...
    x_skip1 = Conv2D(sz, (5,1), activation='linear', padding='same', strides=(1,1))(x_linear)
    x_skip1 = BatchNormalization()(x_skip1)
    x_skip1 = Mish()(x_skip1)
    x_skip1 = Conv2D(sz, (1,5), activation='linear', padding='same', strides=(1,1))(x_skip1)
    
    x_skip2 = Conv2D(sz, 3, activation='linear', padding='same', strides=(1,1))(x_linear)
    
    xi  = Conv2D(sz, filter_sz, activation='linear', padding='same')(xi)
    xi  = BatchNormalization()(xi)
    
    xi  = Add()([xi,x_linear, x_skip1, x_skip2]) #concate
    return xi

def create_model_resnet_multi():
    print("Model cus resnet multi\n--------------------------------------------------------")
    ...
    x = res_block_mish_multi(x,32, pooling=True)
    x = res_block_mish_multi(x,32)
    
    x = res_block_mish_multi(x,64, down=True, up_size=True)
    x = res_block_mish_multi(x,64)
                               
    x = res_block_mish_multi(x,128, down=True, up_size=True)
    x = res_block_mish_multi(x,128)
    
    x = res_block_mish_multi(x,256, up_size=True)
    x = res_block_mish_multi(x,256)
    x = Mish()(x)
...
def load_model(path, custom_objects={}, verbose=0): 
    ...
    model = create_model_resnet_multi()
#     model.summary()
...
#             train_data = read_data("/data/anhnt/utvm_data_split/new_1008/bienso_train_T10_clear.txt")
#             test_data = read_data("/data/anhnt/utvm_data_split/new_1008/bienso_test_T10_clear.txt")
...
if __name__ == '__main__':
    main()
''' 
python utvm-train-resnet-multi.py --model models_official/utvm_models_resnet_multi/utvm-model-stanford-resnet-multi_backup_max --name utvm-model-stanford-resnet-multi --output-dir models_official/utvm_models_resnet_multi -op Adam -lr .0001 -its 100000 -bs 32

models_official/utvm_models_resnet_multi/utvm-model-stanford-resnet-multi_backup_max
