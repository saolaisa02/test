...
from utils_dp import main, allocate_buffers

TRT_LOGGER = trt.Logger()

class alpr_unconstrained_detector():
    def __init__(self):
        self.engine_path = './plate288.engine'
...
#        self.buffers = allocate_buffers(self.engine, 1)
    ...
    def predict_on_batch(self, image_src):
        ...
        while n_left > 0:
            ...
            Yr_batch = main(self.engine, self.context, image_src[start_index:end_index], self.image_size)
        ...
        return np.array(Yr)
model = alpr_unconstrained_detector()
def eval2(eval_data_generator,iou_thres=0.5): 
    ...
    for i,data in enumerate(eval_data_generator):
        ...
        Yr_batch = model.predict_on_batch(Xtrain)
#         print(Yr_batch.shape)
#         return
...
#             print("predict_labels: ",np.shape(predict_labels))
