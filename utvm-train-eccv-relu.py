...
def compute_ap(recall, precision):
    ...
    return ap 
def eval2(model, eval_data_generator,iou_thres=0.65): 
...
def conv_batch_relu(_input,fsz,csz,activation='relu',padding='same',strides=(1,1)):
...
def create_model_eccv():
    print("Origin model with relu\n--------------------------------------------------------------------")
    input_layer = Input(shape=(None,None,3),name='input')

    x = conv_batch_relu(input_layer, 16, 3)
    x = conv_batch_relu(x, 16, 3)
    x = MaxPooling2D(pool_size=(2,2))(x)
    x = conv_batch_relu(x, 32, 3)
    x = res_block(x, 32)
    x = MaxPooling2D(pool_size=(2,2))(x)
...
if __name__ == '__main__':
    main()
''' 
python utvm-train-eccv-relu.py --model models_official/utvm_models_eccv_relu_privatedata/utvm-model-original-relu-privatedata_backup_max --name utvm-model-eccv-relu-privatedata-065 --output-dir models_official/utvm_models_eccv_relu_privatedata_065 -op Adam -lr .0001 -its 100000 -bs 32

python utvm-train-eccv-relu.py --name utvm-model-original-relu-privatedata --output-dir models_official/utvm_models_eccv_relu_privatedata -op Adam -lr .0001 -its 100000 -bs 32
